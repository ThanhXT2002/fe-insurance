...
export abstract class BaseStoreSignal<S> {
  ...
  patch(partial: Partial<S>) {
    this._state.update((s) => ({ ...(s as any), ...(partial as any) }) as S);
  }
  ...
  async fetchWithRetry<T>(
    ...
    while (attempt <= retries) {
      try {
        ...
        const shouldRetry = shouldRetryPredicate
          ? shouldRetryPredicate(res)
        ...
        // If shouldRetry is true, fallthrough to retry logic
        lastError = new Error('Empty result, will retry');
      } catch (err: any) {
        lastError = err;
      }

      attempt++;
      // small delay between retries (can be tuned)
